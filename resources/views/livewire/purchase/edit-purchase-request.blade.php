    ...
    <section class="content">
      ...
      <div class="my-3 p-3 bg-body rounded shadow-sm">
          <form>
              ...
              <div class="card-body">
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <div>
                          <input type="hidden" class="form-control" wire:model="pr_status" value="{{ $this->pr_status }}" >
                ...
                <div class="mb-3 row">
                  ...
                  <div class="col-sm-10">
                    ...
                    {{-- nextappr: {{ $nextapproval }} nextreject: {{ $nextreject }} pr_id {{ $pr_id }}  --}}
                    @if ($poreq->task_userid == Auth::user()->id && $this->pr_status == 2 )
                    ...
                    @endif
                    @if ($poreq->task_userid == Auth::user()->id && $this->pr_status == 3)
                      ...
                      @endif
                    @if ($this->pr_status == 4 && Auth::user()->id == $poreq->task_userid)
                      <button type="button" class="btn btn-primary" name="submit" wire:click="received()">Received</button>
                    @endif
                    <button type="button" class="btn btn-warning" name="submit" data-bs-toggle="modal" data-bs-target="#ModalInfo">Info</button>
                  </div>
              ...
              {{-- {{ Auth::User()->id }}
              {{ $approval_id . $this->pr_status, $approval_id . Auth::User()->id }} --}}
          </form>
      </div>
            ...
            <table class="table table-striped">
              <thead>
                ...
                <tr>
                  ...
                  <th class="text-right">Qty Buffer</th>
              ...
              <tbody>
                @foreach ($poreqdet as $item_req)
                  <tr>
                    ...
                    <td class="text-right" style="width: 10px">{{ number_format($item_req->qty_req,0) }}</td>
                    <td class="text-right" style="width: 10px">{{ number_format($item_req->qty_onhand,0) }}</td>
                    <td class="text-right" style="width: 10px">{{ number_format($item_req->qty_buffer,0) }}</td>
                    <td class="text-right" style="width: 10px">{{ number_format($item_req->goods_price,0) }}</td>
                    <td class="text-right" style="width: 10px">{{ number_format($item_req->goods_price * $item_req->qty_req, 0) }}</td>
                    {{-- @if ($this->pr_status == 3)
              ...
              <div class="col invoice-col">
                <table>
                  ...
                  <tr>
                    ...
                    <td>{{ $this->pr_status }}.{{ $this->doc_status }}</td>
                  </tr>
                  <tr>
